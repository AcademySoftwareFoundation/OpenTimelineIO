...
def _walk_reference_chain(item, time, results):
    ...
    elif isinstance(item, avb.components.Filler):
        return results
...
def _extract_timecode_info(mob):
    ...
    """
    timecodes = [track.component for track in mob.tracks
                 if isinstance(track.component, avb.components.Timecode)]
    ...
    if timecode_values_are_same(timecodes):
        ...
        timecode_start = timecode.start
        timecode_length = timecode.length
        ...
        raise otio.exceptions.NotSupportedError(
            ...
            " This is currently not supported by the AVB adapter. Found:"
