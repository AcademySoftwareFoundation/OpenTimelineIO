...
To use:
otio.adapters.write_to_file(timeline, cdl_output_directory, adapter_name='cdl')
"""
import os
...
def convert_to_cdl(timeline_event):
    ...
    color_dec_list = ET.Element("ColorDecisionList", xmlns="urn:ASC:CDL:v1.01")
    color_decision = ET.SubElement(color_dec_list, "ColorDecision")
    color_corr = ET.SubElement(color_decision, "ColorCorrection", id=reel_name)

    sop_node = ET.SubElement(color_corr, "SOPNode")
    ET.SubElement(sop_node, "Slope").text = slope
    ET.SubElement(sop_node, "Offset").text = offset
    ET.SubElement(sop_node, "Power").text = power

    sat_node = ET.SubElement(color_corr, "SATNode")
    ET.SubElement(sat_node, "Saturation").text = saturation

    tree = ET.ElementTree(color_dec_list)
...
def create_cdl_file(timeline_event, output_dir_path):
    try:
        cdl_fp = os.path.join(output_dir_path, timeline_event.name + ".cdl")
        cdl_string = convert_to_cdl(timeline_event)

        with open(cdl_fp, "w") as f:
...
def write_to_file(input_otio, filepath):
    ...
    if os.path.isdir(output_dir_path):
        ...
        err = filepath + \
              " is not a valid directory, please create it and run again."
