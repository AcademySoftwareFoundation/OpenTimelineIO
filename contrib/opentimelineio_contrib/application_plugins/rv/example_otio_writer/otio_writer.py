...
def get_node_otio_metadata(node_name):
    ...
    node_type = rv.commands.nodeType(node_name)
    metadata = {node_type: {}}
    for prop in rv.commands.properties(node_name):
        if prop == otio_prop:
            continue

        property_info = rv.commands.propertyInfo(prop)
        if property_info["type"] == rv.commands.IntType:
            metadata[node_type][prop] = rv.commands.getIntProperty(prop)
        elif property_info["type"] == rv.commands.FloatType:
            metadata[node_type][prop] = rv.commands.getFloatProperty(prop)
        elif property_info["type"] == rv.commands.StringType:
            metadata[node_type][prop] = rv.commands.getStringProperty(prop)
...
def frames_to_rational_times(start_frame, end_frame, fps):
    ...
    duration = otio.opentime.RationalTime.duration_from_start_end_time(
        start_frame, 
        otio.opentime.RationalTime(end_frame, rate=fps)
    )
