...
The idea of a named coordinate space comes from computer graphics rendering systems, which often have a hierarchy of coordinate systems (also called coordinate frames) with known transformations from one to the other.  An artist can describe data in whichever frame makes intuitive or mathematical sense to them while still trusting that the renderer will be able to robustly translate the data into the correct reference space it needs in order to perform its math on the data.
...
coordinate space on a specific object.  It encodes a reference to an object, `target_object` and an enumeration `target_space`, which refers to a specific coordinate space on that target object.  Not every object will have every enumeration, and the class is meant to be generated by convienence methods present on many of the objects in OpenTimelineIO which end in `_space`.

For example, the `Item` class defines a
