...
main(int argc, char* argv[])
{
    ...
    const std::string tmp_dir_path =
        (RUN_STRUCT.FIXED_TMP ? "/var/tmp/ioperftest"
                              : examples::create_temp_dir());

    otio::ErrorStatus err;
    assert(!otio::is_error(err));

    otio::schema_version_map downgrade_manifest = { { "FakeSchema", 3 },
                                                    { "Clip", 1 },
                                                    { "OtherThing", 12000 } };
    ...
    if (RUN_STRUCT.TO_JSON_STRING)
    {
        ...
        const std::string result =
            timeline.value->to_json_string(&err, &downgrade_manifest);
    ...
    if (RUN_STRUCT.TO_JSON_FILE_NO_DOWNGRADE)
    {
        ...
        timeline.value->to_json_file(
            examples::normalize_path(
                tmp_dir_path + "/io_perf_test.nodowngrade.otio"),
            &err,
            {});
        end = std::chrono::steady_clock::now();
        assert(!otio::is_error(err));
