    ...
    otio::ErrorStatus* error_status)
{
    ...
    const std::string temp_file_name =
        examples::create_temp_dir() + "/temp.otio";
    std::stringstream ss;
    ss << "otioconvert" << " -i " << examples::normalize_path(file_name)
       << " -o " << temp_file_name;
    _run_process(ss.str(), error_status);

    // Read the temporary JSON file.
    return dynamic_cast<otio::Timeline*>(
        otio::Timeline::from_json_file(temp_file_name, error_status));
}
    ...
    otio::ErrorStatus*                                        error_status)
{
    ...
    std::stringstream ss;
    ss << "otioconvert" << " -i " << temp_file_name << " -o "
...
public:
    ...
    ~WCharBuffer() { delete[] p; }

    WCHAR* p = nullptr;
};

bool
PythonAdapters::_run_process(
    ...
    otio::ErrorStatus* error_status)
{
            ...
            &pi))
    {
        ...
        error_status->details = "cannot create process: "
                                + std::string(error_buf, lstrlen(error_buf));
        return false;
    }
...
main(int argc, char** argv)
{
    ...
    {
        std::cout
            << "Usage: python_adapters_child_process (inputpath) (outputpath)"
            << std::endl;
        return 1;
    }
