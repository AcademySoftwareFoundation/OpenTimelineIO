...
void
summarize_effects(otio::SerializableObject::Retainer<otio::Item> const& item)
{
    for (auto effect: item.value->effects())
    {
        if (auto freezeFrame = dynamic_cast<otio::FreezeFrame*>(effect.value))
        {
            ...
            auto linearTimeWarp =
                dynamic_cast<otio::LinearTimeWarp*>(effect.value))
        {
            std::cout.precision(2);
            std::cout << "Effect: Linear Time Warp (" << std::fixed
                      << linearTimeWarp->time_scalar() * 100.0 << "%)"
                      << std::endl;
        }
        else
        {
...
summarize_range(
    ...
    otio::TimeRange const&   time_range,
    otio::ErrorStatus const& errorStatus)
{
    ...
    else
    {
        ...
        std::cout << '\t' << label << ": "
                  ...
                  << " - "
                  << time_range.end_time_exclusive().to_timecode(
                         &otimeErrorStatus)
                  << " (Duration: "
                  << time_range.duration().to_timecode(&otimeErrorStatus) << ")"
                  << std::endl;
    }
}
    ...
    for (const auto i: timeline.value->tracks()->children())
    {
        if (auto track = dynamic_cast<otio::Track*>(i.value))
        {
            ...
            std::cout << "Track: " << track->name() << '\n'
                      << "\tKind: " << track->kind() << '\n'
                      << "\tDuration: "
                      << track->duration(&errorStatus).to_time_string()
                      << std::endl;
            ...
            for (auto child: track->children())
            {
                if (auto item = dynamic_cast<otio::Item*>(child.value))
                {
                    if (auto clip = dynamic_cast<otio::Clip*>(item))
                    {
                        ...
                        summarize_range(
                            "  Trimmed Range",
                            clip->trimmed_range(&errorStatus),
                            errorStatus);
                        summarize_range(
                            "  Visible Range",
                            clip->visible_range(&errorStatus),
                            errorStatus);
                        summarize_range(
                            "Available Range",
                            clip->available_range(&errorStatus),
                            errorStatus);
                    }
                        ...
                        auto transition = dynamic_cast<otio::Transition*>(item))
                    {
                        ...
                        std::cout
                            ...
                            << '\n'
                            << "\tDuration: "
                            << transition->duration(&errorStatus)
                                   .to_timecode(&otimeErrorStatus)
                            << std::endl;
                    }
                    else if (
                        auto composition =
                            dynamic_cast<otio::Composition*>(item))
                    {
                        std::cout
                            << "Nested Composition: " << composition->name()
                            << '\n'
                            << "\tDuration: "
                            << composition->duration(&errorStatus)
                                   .to_time_string()
                            << std::endl;
                    }
                    else
                    {
                        std::cout
                            ...
                            << typeid(item).name() << ")" << '\n'
                            << "\tDuration: "
                            << item->duration(&errorStatus).to_time_string()
                            << std::endl;
                    }
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        otio::SerializableObject::Retainer<otio::Timeline> timeline(
            dynamic_cast<otio::Timeline*>(
                otio::Timeline::from_json_file(argv[i], &error_status)));
        if (!timeline)
        {
