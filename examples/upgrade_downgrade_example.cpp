...
protected:
    ...
    virtual bool read_from(Reader& reader) override
    {
        auto result =
...
main(int argc, char* argv[])
{
    ...
    auto sc =
        otio::SerializableObject::Retainer<SimpleClass>(new SimpleClass());
    sc->set_new_field(12);

    // write it out to disk, without changing it
    sc->to_json_file("/var/tmp/simpleclass.otio", &err);

    otio::schema_version_map downgrade_manifest = { { "SimpleClass", 1 } };
