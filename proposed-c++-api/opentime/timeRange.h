#ifndef OTIO_TIMERANGEH
#define OTIO_TIMERANGEH
...
public:
    explicit TimeRange()
        : _start_time(RationalTime()), _duration(RationalTime())
    {
    }
    ...
    TimeRange extended_by(TimeRange other) const {
        RationalTime new_start_time = std::min(_start_time, other._start_time),
                     ...
                     duration = RationalTime::duration_from_start_end_time(new_start_time, new_end_time);
        return TimeRange(new_start_time, duration);
    }
    ...
    TimeRange clamped(TimeRange other) const {
        TimeRange r = TimeRange(std::max(other._start_time, _start_time),
                                other._duration);
