...
TEST_F(OpenTimeTests, DurationFromStartEndTimeTest) {
  ...
  RationalTime *result = RationalTime_duration_from_start_end_time(startTime, endTime);
...
TEST_F(OpenTimeTests, Timecode24_Test){
  ...
  RationalTime* fromTimeCode = RationalTime_from_timecode(timecode, 24, errorStatus);
...
TEST_F(OpenTimeTests, Timecode23976fps_Test){
  ...
  RationalTime* fromTimeCode = RationalTime_from_timecode(timecode, 23.976, errorStatus);
  EXPECT_EQ(RationalTime_value(t), RationalTime_value(fromTimeCode));
  EXPECT_EQ(RationalTime_rate(t), RationalTime_rate(fromTimeCode));
  RationalTime_destroy(t);
  RationalTime_destroy(fromTimeCode);
  ...
  fromTimeCode = RationalTime_from_timecode(timecode, 24000.0/1001.0, errorStatus);
  EXPECT_EQ(RationalTime_value(t), RationalTime_value(fromTimeCode));
  EXPECT_EQ(RationalTime_rate(t), RationalTime_rate(fromTimeCode));
  RationalTime_destroy(t);
  RationalTime_destroy(fromTimeCode);
...
TEST_F(OpenTimeTests, TimecodeNTSC2997fps_Test){
  ...
  const char* dftc = RationalTime_to_timecode(t, rate_float, ForceYes, errorStatus);
  EXPECT_STREQ(dftc, "10:03:00;05");

  const char* tc = RationalTime_to_timecode(t, rate_float, ForceNo, errorStatus);
  EXPECT_STREQ(tc, "10:02:23:29");

  /* Detect DFTC from rate for backward compatibility with old versions */
  const char* tc_auto = RationalTime_to_timecode(t, rate_float, InferFromRate, errorStatus);
...
TEST_F(OpenTimeTests, Timecode2997Test){
  ErrorStatus* errorStatus = ErrorStatus_create();
  int ref_values_val [6] = {10789, 10790, 17981,
                           17982, 17983, 17984};
  const char* ref_values_tc [6] = {"00:05:59:19", "00:05:59:20", "00:09:59:11",
                                  "00:09:59:12", "00:09:59:13", "00:09:59:14"};
  const char* ref_values_dftc [6] = {"00:05:59;29", "00:06:00;02", "00:09:59;29",
                                    "00:10:00;00", "00:10:00;01", "00:10:00;02"};
  const char* to_dftc = "";
  const char* to_tc = "";
  ...
  for(int i = 0 ; i < 6; i++){
    ...
    to_auto_tc = RationalTime_to_timecode(t, 29.97, InferFromRate, errorStatus);
...
TEST_F(OpenTimeTests, TimeString24Test){
  ...
  RationalTime* time_obj = RationalTime_from_time_string(time_string, 24, errorStatus);
...
TEST_F(OpenTimeTests, TimeString25Test){
  ...
  RationalTime* time_obj = RationalTime_from_time_string(time_string, 25, errorStatus);
  EXPECT_TRUE(RationalTime_almost_equal(t, time_obj, 0.001));
  RationalTime_destroy(t);
  RationalTime_destroy(time_obj);
  t = time_obj = NULL;
...
TEST_F(OpenTimeTests, TimeString23976fpsTest){
  ...
  const char* ref_values_ts [16] = {"00:00:01.708333", "00:04:59.833333",
                                    "00:05:00.0", "00:10:00.0",
                                    "00:20:00.0", "00:29:58.833333",
                                    "00:30:00.0", "00:30:00.25",
                                    "00:40:00.0", "00:50:00.0",
                                    "00:54:57.916666", "00:55:00.0",
                                    "00:01:17.833333", "00:06:16.583333",
                                    "00:12:07.333333", "00:19:32.25"};
  RationalTime* t = NULL;
  for (int i = 0; i < 16; i++) {
