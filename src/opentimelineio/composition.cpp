    ...
    int index, Composable* child, ErrorStatus* error_status)
{
    ...
    if(index >= int(_children.size())) { _children.emplace_back(child); }
    else
    {
...
Composition::is_parent_of(Composable const* other) const
{
    ...
    if(cur_parent == this) return true;
    ...
    Composable const* child, ErrorStatus* error_status) const
{
    ...
    {
        if(_children[i] == child) { return int(i); }
    }
    ...
    Composable const* child, ErrorStatus* error_status) const
{
    ...
    if(*error_status) { return TimeRange(); }
...
{
    if(!source_range()) { return child_range; }
    ...
    if(past_end_time || before_start_time) { return nullopt; }
    ...
    bool                shallow_search) const
{
    ...
    if(!result || shallow_search || !composition) { return result; }
