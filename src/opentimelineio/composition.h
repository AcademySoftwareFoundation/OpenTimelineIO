...
public:
    ...
    virtual TimeRange
    range_of_child_at_index(int index, ErrorStatus* error_status) const;
    virtual TimeRange
    trimmed_range_of_child_at_index(int index, ErrorStatus* error_status) const;

    // leaving out reference_space argument for now:
    TimeRange
    range_of_child(Composable const* child, ErrorStatus* error_status) const;
    ...
    range_of_all_children(ErrorStatus* error_status) const;
...
protected:
    ...
    int
    _index_of_child(Composable const* child, ErrorStatus* error_status) const;
    std::vector<Composition*>
    _path_from_child(Composable const* child, ErrorStatus* error_status) const;

private:
    // XXX: python implementation is O(n^2) in number of children
    std::vector<Composable*>
    _children_at_time(RationalTime, ErrorStatus* error_status) const;
    ...
    if(search_range)
    {
        ...
        if(!error_status)
        {
            *error_status = ErrorStatus(
                ...
                "one or more invalid children encountered");
        }
    }
    else
    {
