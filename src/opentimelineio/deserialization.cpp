...
public:
    ...
    bool has_errored() { return bool(_error_status); }

    void finalize()
    {
        if(!has_errored()) { _resolver.finalize(_error_function); }
    }
    ...
    bool store(any&& a)
    {
        ...
        if(_stack.empty()) { _root.swap(a); }
        else
        {
            auto& top = _stack.back();
            if(top.is_dict) { top.dict.emplace(_stack.back().cur_key, a); }
            else
            {
...
SerializableObject::Reader::_error(ErrorStatus const& error_status)
{
    ...
    _error_function(ErrorStatus(
        ...
        string_printf(
            ...
            name.c_str(),
            demangled_type_name(_source).c_str(),
...
SerializableObject::Reader::_fetch(std::string const& key, double* dest)
{
    ...
    _error(ErrorStatus(
        ...
        string_printf(
            "expected type %s under key '%s': found type %s instead",
            demangled_type_name(typeid(double)).c_str(),
            key.c_str(),
            demangled_type_name(e->second.type()).c_str())));
    return false;
}
...
SerializableObject::Reader::_fetch(std::string const& key, int64_t* dest)
{
    ...
    _error(ErrorStatus(
        ...
        string_printf(
            "expected type %s under key '%s': found type %s instead",
            demangled_type_name(typeid(int64_t)).c_str(),
            key.c_str(),
            demangled_type_name(e->second.type()).c_str())));
    return false;
}
    ...
    if(wanted != found)
    {
        _error(ErrorStatus(
            ...
            string_printf(
                ...
                demangled_type_name(wanted).c_str(),
...
SerializableObject::Reader::_decode(_Resolver& resolver)
{
    ...
    if(!_fetch("OTIO_SCHEMA", &schema_name_and_version)) { return any(); }
    ...
    {
        std::string ref_id;
        if(!_fetch("id", &ref_id)) { return any(); }
    ...
    else
    {
        ...
        {
            if(!_fetch("OTIO_REF_ID", &ref_id)) { return any(); }
        ...
        if(SerializableObject* so = r._instance_from_schema(
               ...
               &error_status))
        {
            if(!ref_id.empty()) { resolver.object_for_id[ref_id] = so; }
...
{
    bool had_null;
    if(!_fetch(key, value, &had_null)) { return false; }

    if(had_null) { value->clear(); }
    return true;
}
    ...
    std::string const& input, any* destination, ErrorStatus* error_status)
{
    ...
    if(handler.has_errored(error_status)) { return false; }
    ...
    std::string const& file_name, any* destination, ErrorStatus* error_status)
{
    FILE* fp = nullptr;
#if defined(_WIN32)
    if(fopen_s(&fp, file_name.c_str(), "r") != 0) { fp = nullptr; }
#else
