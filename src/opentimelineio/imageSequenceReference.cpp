...
{
    if(!this->available_range().has_value()) { return _start_frame; }
...
{
    if(!this->available_range().has_value()) { return 0; }
    ...
    int const image_number, ErrorStatus* error_status) const
{
    if(_rate == 0)
    {
        *error_status = ErrorStatus(
            ErrorStatus::ILLEGAL_INDEX, "Zero rate sequence has no frames.");
        return std::string();
    }
    else if(
