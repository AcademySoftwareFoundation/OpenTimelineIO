...
public:
    ...
    std::string const& target_url_base() const { return _target_url_base; }
    ...
    std::string const& name_prefix() const { return _name_prefix; }
    ...
    std::string const& name_suffix() const { return _name_suffix; }
    ...
    int start_frame() const { return _start_frame; }

    void set_start_frame(int const start_frame) { _start_frame = start_frame; }

    int frame_step() const { return _frame_step; }

    void set_frame_step(int const frame_step) { _frame_step = frame_step; }

    double const& rate() const { return _rate; }

    void set_rate(double const rate) { _rate = rate; }

    int frame_zero_padding() const { return _frame_zero_padding; }
    ...
    int
    frame_for_time(RationalTime const time, ErrorStatus* error_status) const;

    std::string target_url_for_image_number(
        int const image_number, ErrorStatus* error_status) const;
