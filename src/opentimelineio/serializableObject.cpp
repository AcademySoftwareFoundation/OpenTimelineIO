...
SerializableObject::read_from(Reader& reader)
{
    ...
    for(auto& e: reader._dict)
    {
        ...
        if(it != _dynamic_fields.end()) { it->second.swap(e.second); }
        else
        {
...
SerializableObject::_managed_retain()
{
    {
        std::lock_guard<std::mutex> lock(_mutex);
        if(_managed_ref_count++ != 1 || !_external_keepalive_monitor) return;
    }
    ...
    std::function<void()> monitor, bool apply_now)
{
    ...
    if(apply_now) { _external_keepalive_monitor(); }
}
