...
public:
    ...
    from_json_file(std::string const& file_name, ErrorStatus* error_status);
    static SerializableObject*
    from_json_string(std::string const& input, ErrorStatus* error_status);
    ...
    public:
        ...
        template <typename T> bool read(std::string const& key, T* dest)
        {
            any a;
        ...
        bool read(std::string const& key, Retainer<T>* dest)
        {
            ...
            _error(ErrorStatus(
                ...
                string_printf(
                    "Expected object of type %s; read type %s instead",
    ...
    private:
        ...
        {
            if(!_type_check(typeid(AnyVector), source.type())) { return false; }
        ...
        bool _from_any(any const& source, std::map<std::string, T>* dest)
        {
            ...
            {
                T elem;
                if(!_from_any(e.second, &elem)) { break; }
        ...
        template <typename T> bool _from_any(any const& source, T** dest)
        {
        ...
        template <typename T> bool _from_any(any const& source, T* dest)
        {
            if(!_type_check(typeid(T), source.type())) { return false; }
    ...
    public:
        ...
        template <typename T> void write(std::string const& key, T const& value)
        {
    ...
    private:
        ...
        template <typename T> static any _to_any(std::vector<T> const& value)
        {
            AnyVector av;
        ...
        template <typename T> static any _to_any(std::list<T> const& value)
        {
            AnyVector av;
            av.reserve(value.size());
        ...
        template <typename T> static any _to_any(T const* value)
        {
            SerializableObject* so = (SerializableObject*) value;
            return any(SerializableObject::Retainer<>(so));
        }

        template <typename T> static any _to_any(T* value)
        {
            SerializableObject* so = (SerializableObject*) value;
            return any(SerializableObject::Retainer<>(so));
        }

        template <typename T> static any _to_any(Retainer<T> const& value)
        {
    ...
    template <typename T> struct Retainer
    {
        ...
        {
            if(value) value->_managed_retain();
        }
        ...
        {
            if(rhs.value) rhs.value->_managed_retain();
            if(value) value->_managed_release();
            value = rhs.value;
            return *this;
        }
