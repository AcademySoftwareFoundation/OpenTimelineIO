...
template <typename RapidJSONWriterType> class JSONEncoder : public Encoder
{
public:
    JSONEncoder(RapidJSONWriterType& writer) : _writer(writer) {}
...
SerializableObject::Writer::_any_array_equals(any const& lhs, any const& rhs)
{
    ...
    {
        if(!_any_equals(lv[i], rv[i])) { return false; }
    }
...
{
    if(&key != &_no_key) { _encoder.write_key(key); }
}
    ...
    std::string const& key, SerializableObject const* value)
{
    ...
    if(valueEntry != _id_for_object.end()) { _id_for_object.erase(valueEntry); }
#endif
}
...
SerializableObject::Writer::write(std::string const& key, any const& value)
{
    ...
    if(e != _write_dispatch_table.end()) { e->second(value); }
    else
    {
        std::string s;
...
{
    if(_type_record() != other._type_record()) { return false; }
...
SerializableObject::clone(ErrorStatus* error_status) const
{
    ...
    if(e.has_errored(error_status)) { return nullptr; }

    std::function<void(ErrorStatus const&)> error_function =
        [error_status](ErrorStatus const& status) { *error_status = status; };
