...
{
    if(track_index < 0) { track_index = int(tracks.size()) - 1; }
    ...
    if(it != range_track_map.end()) { track_map = &it->second; }
    else
    {
        ...
        if(!item)
        {
            if(!dynamic_retainer_cast<Transition>(child))
            {
                *error_status = ErrorStatus(
                    ...
                    "expected item of type Item* or Transition*",
                    child);
                return;
            }
        }
    ...
    if(track_retainer) { range_track_map.erase(track_retainer); }
}
...
flatten_stack(Stack* in_stack, ErrorStatus* error_status)
{
    ...
    for(auto c: in_stack->children())
    {
        ...
        {
            *error_status = ErrorStatus(
                ErrorStatus::TYPE_MISMATCH, "expected item of type Track*", c);
            return nullptr;
        }
    }
