...
#if defined(__GNUC__) || defined(__clang__)
std::string
cxxabi_demangled_type_name(const char* name)
{
...
std::string
demangled_type_name(std::type_info const& t)
{
    if(t == typeid(std::string)) { return "string"; }
    if(t == typeid(void)) { return "None"; }
...
std::string
demangled_type_name(any const& a)
{
    return demangled_type_name(a.type());
}

std::string
demangled_type_name(SerializableObject* so)
{
