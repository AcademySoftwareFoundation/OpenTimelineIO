...
IsDropFrameRate
df_enum_converter(std::optional<bool>& df)
{
    if (!df.has_value())
    {
...
std::string
opentime_python_str(RationalTime rt)
{
    return string_printf("RationalTime(%g, %g)", rt.value(), rt.rate());
}

std::string
opentime_python_repr(RationalTime rt)
{
    return string_printf(
        "otio.opentime.RationalTime(value=%g, rate=%g)",
        rt.value(),
        rt.rate());
}
...
For example, the duration of a clip from frame 10 to frame 15 is 6 frames. Result will be in the rate of start_time.
        ...
        .def_static(
            "is_smpte_timecode_rate",
            &RationalTime::is_smpte_timecode_rate,
            "rate"_a,
        ...
        .def_static(
            ...
            &RationalTime::nearest_smpte_timecode_rate,
            "rate"_a,
        ...
        .def(
            "to_timecode",
            [](RationalTime rt, double rate, std::optional<bool> drop_frame) {
        ...
        .def(
            "__lt__",
            [](RationalTime lhs, py::object const& rhs) {
