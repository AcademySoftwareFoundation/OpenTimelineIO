...
struct ErrorStatusConverter
{
    ...
    {
        namespace py = pybind11;
        if(error_status) { throw py::value_error(error_status.details); }
    }
...
IsDropFrameRate
df_enum_converter(py::object& df)
{
    if(df.is(py::none())) { return IsDropFrameRate::InferFromRate; }
...
opentime_python_repr(RationalTime rt)
{
    return string_printf(
        "otio.opentime.RationalTime(value=%g, rate=%g)", rt.value(), rt.rate());
}
