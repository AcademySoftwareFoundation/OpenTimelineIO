...
void
opentime_timeRange_bindings(py::module m)
{
    py::class_<TimeRange>(m, "TimeRange")
        // matches the python constructor behavior
        .def(
            py::init([](RationalTime* start_time, RationalTime* duration) {
...
15.5, because the last time with data in this range is 15.
        ...
        .def(
            "extended_by",
            &TimeRange::extended_by,
            "other"_a,
            R"docstring(Construct a new TimeRange that is  this one extended by other.)docstring")
        .def(
            "clamped",
...
::
        ...
        .def(
            ...
            (bool (TimeRange::*)(TimeRange, double) const) &
                TimeRange::contains,
            "other"_a,
            "epsilon_s"_a = opentime::DEFAULT_EPSILON_s,
            R"docstring(
...
The converse would be ``other.finishes(this)``
        ...
        .def("__repr__", [](TimeRange tr) {
            return string_printf(
                "otio.opentime.TimeRange(start_time=%s, duration=%s)",
