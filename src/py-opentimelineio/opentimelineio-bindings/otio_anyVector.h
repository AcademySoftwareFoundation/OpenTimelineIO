...
struct AnyVectorProxy : public AnyVector::MutationStamp
{
    ...
    struct Iterator
    {
        Iterator(MutationStamp& s) : mutation_stamp(s), it(0) {}
        ...
        py::object next()
        {
            if(!mutation_stamp.any_vector) { throw_array_was_deleted(); }
    ...
    py::object get_item(int index)
    {
        ...
        if(index < 0 || index >= int(v.size())) { throw py::index_error(); }
    ...
    void insert(int index, PyAny* pyAny)
    {
        ...
        if(size_t(index) >= v.size()) { v.emplace_back(std::move(pyAny->a)); }
        else
        {
    ...
    {
        AnyVector& v = fetch_any_vector();
        if(v.empty()) { throw py::index_error(); }
    ...
    {
        if(!any_vector) { throw_array_was_deleted(); }
        return *any_vector;
    }
};
