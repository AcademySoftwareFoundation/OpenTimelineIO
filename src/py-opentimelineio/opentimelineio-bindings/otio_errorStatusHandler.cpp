...
ErrorStatusHandler::~ErrorStatusHandler() noexcept(false)
{
    ...
    switch(error_status.outcome)
    {
        ...
        case ErrorStatus::JSON_PARSE_ERROR:
            throw py::value_error(
                "JSON parse error while reading: " + details());
        case ErrorStatus::FILE_OPEN_FAILED:
            throw py::value_error(
                "failed to open file for reading: " + details());
        case ErrorStatus::FILE_WRITE_FAILED:
            throw py::value_error(
                "failed to open file for writing: " + details());
        ...
        case ErrorStatus::OBJECT_CYCLE:
            throw py::value_error(
                ...
                details());
        default: throw py::value_error(full_details());
    }
}
...
{
    if(!error_status.object_details) { return error_status.details; }

    std::string object_str =
        py::cast<std::string>(py::str(py::cast(error_status.object_details)));
    return string_printf(
        "%s: %s", error_status.details.c_str(), object_str.c_str());
}

std::string
ErrorStatusHandler::full_details()
{
    if(!error_status.object_details) { return error_status.full_description; }

    std::string object_str =
        py::cast<std::string>(py::str(py::cast(error_status.object_details)));
    return string_printf(
        "%s: %s", error_status.full_description.c_str(), object_str.c_str());
}

void
otio_exception_bindings(py::module m)
{
    auto otio_exception = py::register_exception<OTIOException>(m, "OTIOError");
    py::register_exception<_NotAChildException>(
        m, "NotAChildError", otio_exception.ptr());
    py::register_exception<_UnsupportedSchemaException>(
        m, "UnsupportedSchemaError", otio_exception.ptr());
    py::register_exception<_CannotComputeAvailableRangeException>(
        m, "CannotComputeAvailableRangeError", otio_exception.ptr());
}
