...
using MarkerVectorProxy = MutableSequencePyAPI<
    std::vector<SerializableObject::Retainer<Marker>>,
    Marker*>;

using EffectVectorProxy = MutableSequencePyAPI<
    std::vector<SerializableObject::Retainer<Effect>>,
    Effect*>;

using TrackVectorProxy = MutableSequencePyAPI<
    std::vector<SerializableObject::Retainer<Track>>,
    Track*>;
...
{
const std::string
string_or_none_converter(py::object& thing)
{
    if(thing.is(py::none())) { return std::string(); }
    else
    {
        return py::str(thing);
    }
    ...
    std::vector<SerializableObject*>& l)
{
    ...
    {
        for(const auto& child: t->template children_if<U>(
                ErrorStatusHandler(), search_range, shallow_search))
        {
...
{
    std::vector<SerializableObject*> l;
    if(children_if<T, Clip>(
           t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Composition>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Gap>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Item>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Stack>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Timeline>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Track>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else if(children_if<T, Transition>(
                t, descended_from_type, search_range, shallow_search, l))
        ;
    else
    {
        for(const auto& child: t->template children_if<Composable>(
                ErrorStatusHandler(), search_range, shallow_search))
        {
            l.push_back(child.value);
        }
...
std::vector<SerializableObject*>
clip_if(
    T* t, optional<TimeRange> const& search_range, bool shallow_search = false)
{
    std::vector<SerializableObject*> l;
    for(const auto& child:
        t->clip_if(ErrorStatusHandler(), search_range, shallow_search))
    {
        l.push_back(child.value);
    }
    return l;
...
template <typename CONTAINER, typename ITEM> class ContainerIterator
{
public:
    ContainerIterator(CONTAINER* container) : _container(container), _it(0) {}
...
static void
define_bases1(py::module m)
{
    py::class_<SerializableObject, managing_ptr<SerializableObject>>(
        m, "SerializableObject", py::dynamic_attr())
        .def(py::init<>())
        ...
        .def(
            "is_equivalent_to",
            &SerializableObject::is_equivalent_to,
            "other"_a.none(false))
        .def(
            "clone",
        ...
        .def(
            "to_json_string",
            [](SerializableObject* so, int indent) {
        ...
        .def(
            ...
            [](SerializableObject* so, std::string file_name, int indent) {
                return so->to_json_file(
                    file_name, ErrorStatusHandler(), indent);
            },
            "file_name"_a,
            "indent"_a = 4)
        .def_static(
        ...
        .def_static(
            ...
            [](std::string input) {
                return SerializableObject::from_json_string(
                    input, ErrorStatusHandler());
            },
            "input"_a)
        .def("schema_name", &SerializableObject::schema_name)
        .def("schema_version", &SerializableObject::schema_version)
        .def_property_readonly(
            "is_unknown_schema", &SerializableObject::is_unknown_schema);

    py::class_<UnknownSchema, SerializableObject, managing_ptr<UnknownSchema>>(
        m, "UnknownSchema")
        .def_property_readonly(
            "original_schema_name", &UnknownSchema::original_schema_name)
        .def_property_readonly(
            "original_schema_version", &UnknownSchema::original_schema_version);

    py::class_<
        SOWithMetadata,
        SerializableObject,
        managing_ptr<SOWithMetadata>>(
        m, "SerializableObjectWithMetadata", py::dynamic_attr())
        .def(
            py::init([](std::string name, py::object metadata) {
...
static void
define_bases2(py::module m)
{
    MarkerVectorProxy::define_py_class(m, "MarkerVector");
    EffectVectorProxy::define_py_class(m, "EffectVector");

    py::class_<Composable, SOWithMetadata, managing_ptr<Composable>>(
        m, "Composable", py::dynamic_attr())
        .def(
            py::init([](std::string const& name, py::object metadata) {
    ...
    auto marker_class =
        py::class_<Marker, SOWithMetadata, managing_ptr<Marker>>(
            m, "Marker", py::dynamic_attr())
            .def(
                py::init([](py::object         name,
    ...
    py::class_<Marker::Color>(marker_class, "Color")
        .def_property_readonly_static(
            "PINK", [](py::object /* self */) { return Marker::Color::pink; })
        .def_property_readonly_static(
            "RED", [](py::object /* self */) { return Marker::Color::red; })
        .def_property_readonly_static(
            "ORANGE",
        ...
        .def_property_readonly_static(
            "GREEN", [](py::object /* self */) { return Marker::Color::green; })
        .def_property_readonly_static(
            "CYAN", [](py::object /* self */) { return Marker::Color::cyan; })
        .def_property_readonly_static(
            "BLUE", [](py::object /* self */) { return Marker::Color::blue; })
        .def_property_readonly_static(
            "PURPLE",
        ...
        .def_property_readonly_static(
            "BLACK", [](py::object /* self */) { return Marker::Color::black; })
    ...
    py::class_<SerializableCollectionIterator>(
        m, "SerializableCollectionIterator", py::dynamic_attr())
    ...
    py::class_<
        ...
        SOWithMetadata,
        managing_ptr<SerializableCollection>>(
        m, "SerializableCollection", py::dynamic_attr())
        .def(
            py::init([](std::string const& name,
                        py::object         children,
                        py::object         metadata) {
            ...
            metadata_arg)
        .def(
            "__internal_getitem__",
            [](SerializableCollection* c, int index) {
        ...
        .def(
            ...
            [](SerializableCollection* c, int index, SerializableObject* item) {
                index = adjusted_vector_index(index, c->children());
        ...
        .def(
            "__len__",
            [](SerializableCollection* c) { return c->children().size(); })
        .def(
            "__iter__",
            [](SerializableCollection* c) {
        ...
        .def(
            ...
            "descended_from_type"_a = py::none(),
            "search_range"_a        = nullopt);
}

static void
define_items_and_compositions(py::module m)
{
    py::class_<Item, Composable, managing_ptr<Item>>(
        m, "Item", py::dynamic_attr())
        .def(
            py::init([](std::string         name,
                        optional<TimeRange> source_range,
                        py::object          effects,
                        py::object          markers,
                        py::object          metadata) {
                return new Item(
            ...
            metadata_arg)
        .def_property(
            "source_range", &Item::source_range, &Item::set_source_range)
        .def(
            "available_range",
            [](Item* item) {
        ...
        .def(
            "transformed_time",
            [](Item* item, RationalTime t, Item* to_item) {
        ...
        .def(
            ...
            [](Item* item, TimeRange time_range, Item* to_item) {
                return item->transformed_time_range(
                    time_range, to_item, ErrorStatusHandler());
            },
            "time_range"_a,
            "to_item"_a);
    ...
    auto transition_class =
        py::class_<Transition, Composable, managing_ptr<Transition>>(
            ...
            .def_property(
                "in_offset", &Transition::in_offset, &Transition::set_in_offset)
            .def_property(
                "out_offset",
    ...
    py::class_<Transition::Type>(transition_class, "Type")
        .def_property_readonly_static(
            "SMPTE_Dissolve",
            [](py::object /* self */) {
                return Transition::Type::SMPTE_Dissolve;
            })
        .def_property_readonly_static("Custom", [](py::object /* self */) {
            return Transition::Type::Custom;
        });
    ...
    py::class_<Composition, Item, managing_ptr<Composition>>(
        m, "Composition", py::dynamic_attr())
        .def(
            py::init([](std::string         name,
                        py::object          children,
                        optional<TimeRange> source_range,
                ...
                Composition* c = new Composition(
                    name, source_range, py_to_any_dictionary(metadata));
                c->set_children(
                    py_to_vector<Composable*>(children), ErrorStatusHandler());
                return c;
            }),
            name_arg,
            "children"_a     = py::none(),
        ...
        .def(
            ...
            [](Composition* c, int index) {
                return c->trimmed_range_of_child_at_index(
                    index, ErrorStatusHandler());
            },
            "index"_a)
        .def(
            "range_of_child",
            [](Composition* c, Composable* child, Composable* /* ignored */) {
                return c->range_of_child(child, ErrorStatusHandler());
            },
            "child"_a,
            "reference_space"_a = nullptr)
        .def(
        ...
        .def(
            "trimmed_child_range",
            &Composition::trim_child_range,
            "child_range"_a)
        .def(
            "trim_child_range", &Composition::trim_child_range, "child_range"_a)
        .def(
        ...
        .def(
            ...
            [](Composition*        t,
               ...
               bool                shallow_search) {
                auto result = t->child_at_time(
                    search_time, ErrorStatusHandler(), shallow_search);
                return result.value;
            },
            "search_time"_a,
            "shallow_search"_a = false)
        .def(
            "children_in_range",
            [](Composition* t, TimeRange const& search_range) {
                std::vector<SerializableObject*> l;
                for(const auto& child:
                    t->children_in_range(search_range, ErrorStatusHandler()))
                {
                    l.push_back(child.value);
                }
                return l;
        ...
        .def(
            ...
            [](Composition*               t,
               ...
               bool                       shallow_search) {
                return children_if(
                    t, descended_from_type, search_range, shallow_search);
            },
            "descended_from_type"_a = py::none(),
            "search_range"_a        = nullopt,
            "shallow_search"_a      = false)
        .def(
            "handles_of_child",
            [](Composition* c, Composable* child) {
                auto result = c->handles_of_child(child, ErrorStatusHandler());
                return py::make_tuple(
                    py::cast(result.first), py::cast(result.second));
            },
            "child_a")
        .def(
            "__internal_getitem__",
        ...
        .def(
            "__internal_setitem__",
            [](Composition* c, int index, Composable* composable) {
                index = adjusted_vector_index(index, c->children());
        ...
        .def("__len__", [](Composition* c) { return c->children().size(); })
        .def("__iter__", [](Composition* c) {
            return new CompositionIterator(c);
        });

    auto track_class = py::class_<Track, Composition, managing_ptr<Track>>(
        m, "Track", py::dynamic_attr());

    py::enum_<Track::NeighborGapPolicy>(track_class, "NeighborGapPolicy")
        .value(
            "around_transitions", Track::NeighborGapPolicy::around_transitions)
    ...
    track_class
        ...
        .def(
            ...
            [](Track* t, Composable* item, Track::NeighborGapPolicy policy) {
                ...
                return py::make_tuple(
                    py::cast(result.first.take_value()),
                    py::cast(result.second.take_value()));
            },
            "item"_a,
            "policy"_a = Track::NeighborGapPolicy::never)
        .def(
            "clip_if",
            [](Track*                     t,
               optional<TimeRange> const& search_range,
    ...
    py::class_<Track::Kind>(track_class, "Kind")
        .def_property_readonly_static(
            "Audio", [](py::object /* self */) { return Track::Kind::audio; })
        .def_property_readonly_static(
            "Video", [](py::object /* self */) { return Track::Kind::video; });

    py::class_<Stack, Composition, managing_ptr<Stack>>(
        m, "Stack", py::dynamic_attr())
        .def(
            py::init([](py::object                 name,
                        py::object                 children,
                        optional<TimeRange> const& source_range,
        ...
        .def(
            "clip_if",
            [](Stack* t, optional<TimeRange> const& search_range) {
                return clip_if(t, search_range);
            },
            "search_range"_a = nullopt);
    ...
    py::class_<
        ...
        managing_ptr<Timeline>>(m, "Timeline", py::dynamic_attr())
        .def(
            py::init([](std::string            name,
                        py::object             children,
                ...
                Timeline* t              = new Timeline(
                    name, global_start_time, py_to_any_dictionary(metadata));
                if(!composable_children.empty())
                    t->tracks()->set_children(
                        composable_children, ErrorStatusHandler());
                return t;
            }),
            name_arg,
            "tracks"_a            = py::none(),
        ...
        .def(
            "clip_if",
            [](Timeline* t, optional<TimeRange> const& search_range) {
                return clip_if(t, search_range);
            },
            "search_range"_a = nullopt)
        .def(
...
static void
define_effects(py::module m)
{
    py::class_<Effect, SOWithMetadata, managing_ptr<Effect>>(
        m, "Effect", py::dynamic_attr())
        .def(
            py::init([](std::string name,
                        std::string effect_name,
                        py::object  metadata) {
                return new Effect(
                    name, effect_name, py_to_any_dictionary(metadata));
            }),
            name_arg,
    ...
    py::class_<TimeEffect, Effect, managing_ptr<TimeEffect>>(
        m, "TimeEffect", py::dynamic_attr())
        .def(
            py::init([](std::string name,
                        std::string effect_name,
                        py::object  metadata) {
    ...
    py::class_<LinearTimeWarp, TimeEffect, managing_ptr<LinearTimeWarp>>(
        m, "LinearTimeWarp", py::dynamic_attr())
        .def(
            py::init(
                [](std::string name, double time_scalar, py::object metadata) {
    ...
    py::class_<FreezeFrame, LinearTimeWarp, managing_ptr<FreezeFrame>>(
        m, "FreezeFrame", py::dynamic_attr())
        .def(
            py::init([](std::string name, py::object metadata) {
...
static void
define_media_references(py::module m)
{
    py::class_<MediaReference, SOWithMetadata, managing_ptr<MediaReference>>(
        m, "MediaReference", py::dynamic_attr())
        .def(
            py::init([](std::string         name,
                        optional<TimeRange> available_range,
                        py::object          metadata) {
                return new MediaReference(
                    name, available_range, py_to_any_dictionary(metadata));
            }),
            name_arg,
        ...
        .def_property_readonly(
            "is_missing_reference", &MediaReference::is_missing_reference);

    py::class_<
        GeneratorReference,
        MediaReference,
        managing_ptr<GeneratorReference>>(
        m, "GeneratorReference", py::dynamic_attr())
        .def(
            py::init([](std::string                name,
    ...
    py::class_<
        MissingReference,
        MediaReference,
        managing_ptr<MissingReference>>(
        m, "MissingReference", py::dynamic_attr())
        .def(
            py::init([](py::object          name,
                        optional<TimeRange> available_range,
                        py::object          metadata) {
    ...
    py::class_<
        ExternalReference,
        MediaReference,
        managing_ptr<ExternalReference>>(
        m, "ExternalReference", py::dynamic_attr())
        .def(
            py::init([](std::string                target_url,
                        optional<TimeRange> const& available_range,
                        py::object                 metadata) {
    ...
    auto imagesequencereference_class = py::class_<
        ...
        m, "ImageSequenceReference", py::dynamic_attr(), R"docstring(
...
- ``file:///show/sequence/shot/sample_image_sequence.0001.exr``
    ...
    imagesequencereference_class
        ...
        .def(
            "end_frame",
            &ImageSequenceReference::end_frame,
        ...
        .def(
            "frame_for_time",
            [](ImageSequenceReference* seq_ref, RationalTime time) {
        ...
        .def(
            ...
            [](ImageSequenceReference* seq_ref, int image_number) {
                return seq_ref->target_url_for_image_number(
                    image_number, ErrorStatusHandler());
            },
            "image_number"_a,
            R"docstring(Given an image number, returns the ``target_url`` for that image.
...
This is roughly equivalent to:
    ``f"{target_url_prefix}{(start_frame + (image_number * frame_step)):0{value_zero_padding}}{target_url_postfix}``
        ...
        .def(
            ...
            "image_number"_a,
            "Given an image number, returns the :class:`RationalTime` at which that image should be shown in the space of `available_range`.");
}

void
otio_serializable_object_bindings(py::module m)
{
    define_bases1(m);
    define_bases2(m);
    define_effects(m);
