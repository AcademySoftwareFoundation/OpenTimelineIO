...
public:
    ...
    bool read_from(Reader& reader) { return Parent::read_from(reader); }

    void write_to(Writer& writer) const { Parent::write_to(writer); }

    void add_key(std::string key, int value) { metadata()[key] = value; }

    std::string repr() const
    {
        return string_printf(
            "<TestObject named '%s' at id %p>",
            name().c_str(),
            this);
    }
...
private:
    ...
    {
        printf(
            "Test object '%s' at %p being destroyed\n",
            name().c_str(),
            this);
    }
};
...
static void
test_takeme(SerializableObject* so)
{
    SerializableObject::Retainer<> r(so);
}

static int
test_bash_retainers1(SerializableCollection* sc)
{
...
py::object
test_bash_retainers2(SerializableCollection* sc, py::object materialize_obj)
{
    SerializableObject* so    = sc->children()[0];
    int                 total = 0;
...
void
test_AnyDictionary_destroy(AnyDictionaryProxy* d)
{
    delete d->any_dictionary;
}

void
test_AnyVector_destroy(AnyVectorProxy* v)
{
    delete v->any_vector;
}

void
otio_tests_bindings(py::module m)
{
    ...
    py::module test =
        m.def_submodule("_testing", "Module for OTIO regression testing");
