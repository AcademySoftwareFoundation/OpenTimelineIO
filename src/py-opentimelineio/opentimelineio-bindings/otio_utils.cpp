...
static std::
    map<std::type_info const*, std::function<py::object(any const&, bool)>>
        _py_cast_dispatch_table;
static std::map<std::string, std::function<py::object(any const&, bool)>>
...
_build_any_to_py_dispatch_table()
{
    ...
    t[&typeid(AnyDictionaryProxy*)] = [](any const& a, bool) {
        return py::cast(any_cast<AnyDictionaryProxy*>(a));
    };
    t[&typeid(AnyVectorProxy*)] = [](any const& a, bool) {
        return py::cast(any_cast<AnyVectorProxy*>(a));
    };
...
static void
py_to_any(py::object const& o, any* result)
{
...
{
    if(o.is_none()) { return AnyDictionary(); }
...
py_to_so_vector(pybind11::object const& o)
{
    ...
    py::object obj_vector = _value_to_so_vector(
        o); // need to retain this here or we'll lose the any...
    AnyVector const& v =
        temp_safely_cast_any_vector_any(obj_vector.cast<PyAny*>()->a);
...
py::object
any_to_py(any const& a, bool top_level)
{
...
struct KeepaliveMonitor
{
    ...
    KeepaliveMonitor(SerializableObject* so) : _so(so) {}
    ...
    void monitor()
    {
        ...
        {
            if(!_keep_alive) { _keep_alive = pybind11::cast(_so); }
        }
        else
        {
            if(_keep_alive)
