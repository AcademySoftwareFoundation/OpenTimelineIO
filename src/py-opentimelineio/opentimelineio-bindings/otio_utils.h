...
{
template <typename T>
struct type_caster<optional<T>> : public optional_caster<optional<T>>
{};
...
{
    managing_ptr(T* ptr) : _retainer(ptr)
    {
...
struct MutableSequencePyAPI : public V
{
    ...
    {
    public:
        Iterator(V& v) : _v(v), _it(0) {}

        Iterator* iter() { return this; }

        VALUE_TYPE next()
        {
            if(_it == _v.size()) { throw pybind11::stop_iteration(); }
    ...
    void insert(int index, VALUE_TYPE value)
    {
        ...
        if(size_t(index) >= v.size()) { v.emplace_back(std::move(value)); }
        else
        {
    ...
    void del_item(int index)
    {
        ...
        if(size_t(index) >= v.size()) { v.pop_back(); }
        else
        {
    ...
    int len() { return static_cast<int>(this->size()); }
...
struct PyAny
{
    ...
    template <typename T>
    PyAny(T& value) : a(create_safely_typed_any(std::move(value)))
    {}

    PyAny(SerializableObject* value) : a(create_safely_typed_any(value)) {}
...
template <typename T>
std::vector<T>
py_to_vector(pybind11::object const& o)
{
    ...
    for(auto e: vso)
    {
        ...
        throw pybind11::type_error(string_printf(
            "list has element of type %s; expected type %s",
