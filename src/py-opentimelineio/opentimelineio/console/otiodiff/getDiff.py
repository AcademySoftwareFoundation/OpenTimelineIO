...
def diffTimelines(timelineA, timelineB):
    ...
    Return an annotated otio file with the differences and print a text summary
    to console.

     Parameters:
     timelineA (otio.schema.Timeline()): timeline from the file you want to
     compare against, ex. clip1 version 1
     timelineB (otio.schema.Timeline()): timeline from the file you want to
     compare, ex. clip1 version 2

     Returns:
     outputTimeline (otio.schema.Timeline()): timeline with color coded clips
     and marker annotations showing the differences between the input tracks
     with the tracks from timeline B stacked on top of timeline A
    '''
    hasVideo = False
    hasAudio = False
    ...
    if hasVideo and hasAudio:
        videoClipTable = categorizeClipsByTracks(
            timelineA.video_tracks(), timelineB.video_tracks())
...
def findClones(clips):
    """Separate the cloned ClipDatas (ones that share the same name) from the
    unique ClipDatas and return both
...
def sortClones(clipDatasA, clipDatasB):
    """Identify cloned ClipDatas (ones that share the same name) across two
    groups of ClipDatas and separate from the unique
...
def compareClones(clonesA, clonesB):
    """Compare two groups of cloned ClipDatas and categorize into
    added, unchanged, or deleted"""
    added = []
    unchanged = []
    deleted = []
...
def sortMoved(clipTable):
    """Put ClipDatas that have moved between tracks into their own category and
    remove from their previous category"""
    allAdd = []
    allEdit = []
    allSame = []
    allDel = []
...
def makeNewOtio(clipTable, trackType):
    """Make a new annotated OTIO timeline showing the change from timeline A to
    timeline B, with the tracks from timeline B stacked on top of
    the tracks from timeline A
    ...
    for trackNum in clipTable.keys():
        ...
        SortedClipDatas = namedtuple(
            'ClipGroup', ['add', 'edit', 'same', 'delete', 'move'])
        clipGroup = SortedClipDatas(clipTable[trackNum]["add"],
                                    clipTable[trackNum]["edit"],
                                    clipTable[trackNum]["same"],
                                    clipTable[trackNum]["delete"],
                                    clipTable[trackNum]["move"])
    ...
    # write order to output timeline so that timeline B is on top for both
    # video and audio
...
def categorizeClipsByTracks(tracksA, tracksB):
    ...
    in tracksA and categorize based on how they have changed.
    Return a dictionary table of ClipDatas categorized by
    added, edited, unchanged, deleted, and moved and ordered by track.
...
def makeSummary(clipTable, trackType, mode):
    ...
    if mode == "summary":
        for track in clipTable.keys():
            ...
            allAdd.extend(clipGroup["add"]
                          ) if "add" in clipGroup.keys() else print("no add")
            allDel.extend(
                clipGroup["delete"]) if "delete" in clipGroup.keys() \
                else print("no del")
            allSame.extend(
                clipGroup["same"]) if "same" in clipGroup.keys() \
                else print("no same")
            allEdit.extend(
                clipGroup["edit"]) if "edit" in clipGroup.keys() \
                else print("no edit")
            allMove.extend(
                clipGroup["move"]) if "move" in clipGroup.keys() \
                else print("no move")
            ...
            for cat in clipGroup.keys():
                ...
                if cat != "same":
                    for i in clipGroup[cat]:
                        print(i.name + ": " + i.note) if i.note is not None \
                            else print(i.name)
    print("")
