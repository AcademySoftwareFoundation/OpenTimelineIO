...
"""Unit tests for the _core_utils module, which provides python ergonomics

NOTE: _core_utils is considered to be internal implementation detail.
"""
...
class AnyDictionaryTests(unittest.TestCase):
    def test_main(self):
        ...
        # TODO: Test different type of values to exercise the any_to_py
        # function?
    ...
    def test_raises_if_ref_destroyed(self):
        d1 = _core_utils.AnyDictionary()
        _otio._testing.test_AnyDictionary_destroy(d1)

        with self.assertRaisesRegex(
            ValueError,
            r"underlying C\+\+ AnyDictionary has been destroyed"
        ):
            d1['asd']

        d2 = _core_utils.AnyDictionary()
        _otio._testing.test_AnyDictionary_destroy(d2)
        ...
        d3 = _core_utils.AnyDictionary()
        _otio._testing.test_AnyDictionary_destroy(d3)
        ...
        d4 = _core_utils.AnyDictionary()
        d4['asd'] = 1
        it = iter(d4)
        _otio._testing.test_AnyDictionary_destroy(d4)
        with self.assertRaisesRegex(
            ValueError,
            r"underlying C\+\+ AnyDictionary has been destroyed"
        ):
...
class AnyVectorTests(unittest.TestCase):
    def test_main(self):
        v = _core_utils.AnyVector()

        with self.assertRaises(IndexError):
            # There is a special case in the C++ code for empty vector
            del v[0]
        ...
        # __add__
        self.assertEqual(v + ['new'], [1, '234', {}, 1, 2, 'new'])

        # __radd__
        self.assertEqual(['new'] + v, [1, '234', {}, 1, 2, 'new'])

        # __add__ with non list type
        self.assertEqual(v + ('new',), [1, '234', {}, 1, 2, 'new'])

        v2 = _core_utils.AnyVector()
        v2.append('v2')

        # __add__ with AnyVector
        self.assertEqual(v + v2, [1, '234', {}, 1, 2, 'v2'])
        ...
        v4 = _core_utils.AnyVector()
        v4.extend(range(10))

        del v4[::2]
        self.assertEqual(list(v4), [1, 3, 5, 7, 9])

        v5 = _core_utils.AnyVector()
        tmplist = [1, 2]
        v5.append(tmplist)

        # If AnyVector was a pure list, this would fail. But it's not a real
        # list. Appending copies data, completely removing references to it.
        self.assertIsNot(v5[0], tmplist)

    def test_raises_if_ref_destroyed(self):
        v1 = _core_utils.AnyVector()
        _otio._testing.test_AnyVector_destroy(v1)

        with self.assertRaisesRegex(
            ValueError,
            r"underlying C\+\+ AnyVector object has been destroyed"
        ):
            v1[0]

        v2 = _core_utils.AnyVector()
        _otio._testing.test_AnyVector_destroy(v2)
        ...
        v3 = _core_utils.AnyVector()
        _otio._testing.test_AnyVector_destroy(v3)
        ...
        v4.append(1)
        it = iter(v4)
        _otio._testing.test_AnyVector_destroy(v4)
        with self.assertRaisesRegex(
            ValueError,
            r"underlying C\+\+ AnyVector object has been destroyed"
        ):
    ...
    def test_copy(self):
        ...
        # AnyVector can only deep copy. So it's __copy__ does a deepcopy.
