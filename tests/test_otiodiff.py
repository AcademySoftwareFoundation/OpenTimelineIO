...
class TestClipData(unittest.TestCase):
    ...
    def test_check_not_same2(self):
        # check that two clips with different source range
        # start durations are not the same
        clipA = otio.schema.Clip(
            name="testName testTake",
            media_reference=otio.core.MediaReference(
                available_range=otio.opentime.TimeRange(
...
class TestGetDif(unittest.TestCase):
    ...
    def test_find_clones(self):
        ...
        assert (clones == correctClones
                ), "Not all cloned clips correctly identified"
        assert (nonClones == correctNonClones
                ), "Not all unique clips correctly identified"
    ...
    def test_sort_clones_clones_in_both(self):
        ...
        assert (len(clonesA) == 1
                ), "Number of clones found in trackA doesn't match"
        assert (len(nonClonesA) == 2
                ), "Number of non-clones found in trackA doesn't match"
        assert (len(clonesB) == 1
    ...
    def test_sort_clones_clones_in_one(self):
        ...
        assert (len(clonesB) == 0
                ), "Number of clones found in trackB doesn't match"
        assert (len(nonClonesB) == 3
                ), "Number of non-clones found in trackB doesn't match"
