...
from opentimelineio.console import (
    ...
    autogen_core_specification as acs
)
...
@unittest.skipIf(
    os.environ.get("OTIO_DISABLE_SERIALIZED_SCHEMA_TEST"),
    "Core specification test disabled because "
    "$OTIO_DISABLE_SERIALIZED_SCHEMA_TEST is set to something other than ''"
)
class CoreSpecificationTester(unittest.TestCase):
    def test_core_specification(self):
        """Test that the generated Core Specification matches the checked-in
        baseline. It is regenerated from the code (schema, types, docstrings)
        and the curated overlay, so it must be updated whenever either changes.
        """

        pt = os.path.dirname(os.path.dirname(__file__))
        fp = os.path.join(pt, "docs", "spec", "otio-core-specification.md")
        overlay = os.path.join(pt, "docs", "spec",
                               "core-specification-overlay.md")
        with open(fp) as fi:
            baseline_text = fi.read()

        test_text = acs.generate_and_write_specification(overlay)
        ...
        self.assertMultiLineEqual(
            baseline_text,
            test_text,
            "\n The core specification has changed and the generated document "
            "in {} needs to be updated.  run: `make spec-update`".format(fp)
        )
